...
feature_columns = list(range(2, 20))

# 要预测的列，按原数据从0开始计算
label_columns = [15, 16, 17, 18, 19]


class Config:
    label_in_feature_index = (lambda x, y: [x.index(i) for i in y])(
        feature_columns, label_columns)  # 因为feature不一定从0开始
    ...
    valid_data_rate = 1 - train_data_rate  # 验证数据占训练数据比例，验证集在训练过程使用，为了做模型和参数选择
    ...
    train_data_path = "./data/" + dataset + ".csv"
    model_save_path = "./checkpoint/" + dataset + "/" + used_frame + "/"
    figure_save_path = "./figure/" + dataset + "/"
    log_save_path = "./log/" + dataset + "/"
    ...
    def read_data(self):  # 读取初始数据
        if self.config.debug_mode:
            ...
            init_data = pd.read_csv(
                self.config.train_data_path, usecols=feature_columns)
...
def draw(config: Config, origin_data: Data, logger, predict_norm_data: np.ndarray):
    ...
    label_column_num = len(label_columns)
    ...
    if not sys.platform.startswith('linux'):
        for i in range(label_column_num):
            ...
            plt.plot(label_X, label_data[:, i], label='label', color='red')
            plt.plot(predict_X, predict_data[:, i], label='predict', color='blue')
...
if __name__ == "__main__":
    ...
    parser.add_argument("-t", "--do_train", default=True,
                        type=bool, help="whether to train")
    parser.add_argument("-p", "--do_predict", default=True,
                        type=bool, help="whether to predict")
    parser.add_argument("-b", "--batch_size", default=64,
                        type=int, help="batch size")
    parser.add_argument("-e", "--epoch", default=20,
                        type=int, help="epochs num")

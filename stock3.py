...
class Config:
    # 数据参数
    feature_columns = list(range(1, 8))  # 要作为feature的列，按原数据从0开始计算，Pos,Neg,AvePos,AveNeg,StdPos,StdNeg
    label_columns = [7]  # 要预测的列，按原数据从0开始计算, 预测第7列，下一天的开盘价
...
def draw(config: Config, origin_data: Data, logger, predict_norm_data: np.ndarray):
    ...
    if not sys.platform.startswith('linux'):  # 无桌面的Linux下无法输出，如果是有桌面的Linux，如Ubuntu，可去掉这一行
        for i in range(label_column_num):
            ...
            plt.plot(label_X, label_data[:, i], label='label', color='r')
            plt.plot(predict_X, predict_data[:, i], label='predict', color='b')
